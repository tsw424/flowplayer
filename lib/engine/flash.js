   ...
   var engine = {
      ...
      load: function(video) {
         ...
         if (api) {
            ...
            // optional conf
            $.each(['key', 'autoplay', 'preload', 'rtmp', 'subscribe', 'live', 'loop', 'debug', 'splash', 'rtmpt'], function(i, key) {
            ...
            setInterval(function () {
               ...
               player.trigger("progress", [player, status.time]);

               if (status.buffer < video.bytes) {
                  video.buffer = status.buffer / video.bytes * video.duration;
                  player.trigger("buffer", [player, video.buffer]);
               } else if (!video.buffered && status.time > 0) {
                  video.buffered = true;
