...
flowplayer.engine.flash = function(player, root) {
   ...
   var engine = {
      ...
      load: function(video) {
         ...
         if (api) {
            ...
            player.on('ready', readyCallback)
                  .on("ready fullscreen fullscreen-exit", fullscreenCallback);
            ...
            // optional conf
            ['key', 'autoplay', 'preload', 'rtmp', 'subscribe', 'live', 'loop', 'debug', 'splash'].forEach(function(key) {
            ...
            setInterval(function () {
               if (!api || !api.__status) return;
               var status = api.__status();
               if (!status) return;

               player.trigger("progress", status.time);

               if (status.buffer < video.bytes && !video.buffered) {
      ...
      unload: function() {
         ...
         api = 0;
         player.off('ready', readyCallback).off('ready fullscreen fullscreen-exit', fullscreenCallback);
      }
