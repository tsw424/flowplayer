...
var engine = function(player, root) {
      ...
      load: function(video) {
         ...
         if (conf.splash && !api) {
           ...
           common.prepend(root, videoTag);
            ...
            if (track) common.append(videoTag, common.attr(track, 'default', ''));
            ...
            api = videoTag;
            if (!api) {
              api = videoTag = createVideoTag(video, false, conf.clip.preload || "metadata");
              common.prepend(root, api);
            } else {
              ClassList(api).add('fp-engine');
              Sizzle('source', api).forEach(common.removeNode);
