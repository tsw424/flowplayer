...
flowplayer(function(api, root) {
   ...
   // aspect ratio
   function setRatio(val, force) {
      if ((common.css(root, 'width') === '0px' || common.css(root, 'height') === '0px') || val !== flowplayer.defaults.ratio || force) {
   ...
   api.on("ready", function(ev, api, video) {
      ...
      conf.adaptiveRatio && setRatio(video.height / video.width, true);
   ...
   // misc
      ...
      if (error) {
         ...
         Sizzle('p', el)[0].innerText = error.url || api.video.url || api.video.src || conf.errorUrls[error.code];
   ...
   if (!has_bg && api.forcedSplash) {
      common.css(root, "backgroundColor", "#555");
   }
