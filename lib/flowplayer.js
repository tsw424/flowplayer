  ...
  var root = element,
      ...
      var api = {
         ...
         load: function(video, callback) {
            ...
            if (video.src) {
               var e = new CustomEvent('load', {detail: {api: api, video: video, engine: engine}});
      ...
      ['on', 'one', 'off'].forEach(function(key) {
        api[key] = function() {
          console.log('key', key);
          var args = [root].concat(Array.slice ? Array.slice(arguments) : Array.prototype.slice.call(arguments));
          console.log('arguments', args);
          bean[key].apply(bean, args);
          return api;
        };
      });

      api.trigger = function(event, arg) {
        bean.fire(root, event, [api, arg]);
        return api;
      };

      //Ugly hack to make load event defaultPreventable
      var loadHandlerHack = function(ev) {
        ev.stopPropagation()
        ev.stopImmediatePropagation()
        ev.preventDefault();
        setTimeout(function() {
          bean.fire(root, 'load', [ev.detail.api, ev.detail.video, ev.detail.engine])
          api.one(loadHandlerHack);
        });
      };
      api.one('load', loadHandlerHack);
