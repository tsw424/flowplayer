...
extend(flowplayer, {
   ...
   defaults: {
      ...
      hlsFix: isSafari && Number(/(\d+\.\d+) Safari/.exec(navigator.userAgent)[1]) < 8
...
if (typeof window.jQuery !== 'undefined') {
  ...
  $.fn.flowplayer = function(opts, callback) {
    return this.each(function() {
      ...
      initializePlayer(this, opts, callback);
    });
  };
}
...
function initializePlayer(element, opts, callback) {
  ...
  var root = element,
      ...
      var isRTL = (root.currentStyle && root.currentStyle['direction'] === 'rtl')
         || (window.getComputedStyle && window.getComputedStyle(root, null).getPropertyValue('direction') === 'rtl');
      ...
      var api = {
         ...
         load: function(video, callback) {
            ...
            if (video.src) {
               var e = new CustomEvent('load', {cancelable: true, bubbles: true, detail: {api: api, additionalArgs: [video, engine]}});
               api.trigger(e);
         ...
         */
         seek: function(time, callback) {
            if (api.ready) {
               ...
               var ev = new CustomEvent('beforeseek', {cancelable: true, detail: {api: api, additionalArgs: [time]}});
               api.trigger(ev, [api, time]);
      ...
      ['on', 'one'].forEach(function(key) {
        api[key] = function(evType, slctr, hndlr) {
          if (!hndlr && isFunction(slctr)) hndlr = slctr;
          bean[key](root, evType, function() {
            if (arguments.length > 1) {
              return hndlr.apply(undefined, Array.slice ? Array.slice(arguments) : Array.prototype.slice.call(arguments));
            }
            // Always give api as second argument
            var ev = arguments[0],
                args = [ev],
                additional = (ev.detail && ev.detail.additionalArgs) || [];
            if (ev.detail && ev.detail.api) args = args.concat([ev.detail.api]).concat(additional);
      ...
      api.trigger = function(event, arg) {
        if (typeof event === 'string') {
          event = new CustomEvent(event, {cancelable: true, bubbles: false, detail: arg});
        }
      ...
      if (!root.getAttribute('data-flowplayer-instance-id')) { // Only bind once
         ...
         api.on('boot', function() {
            ...
            if (engine.pick(conf.clip.sources)) {
