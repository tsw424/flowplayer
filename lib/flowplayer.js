...
var flowplayer = module.exports = function(fn, opts, callback) {
  ...
  if (fn.jquery) return flowplayer(fn[0]);
};
    ...
    return this.each(function() {
      ...
      events.EVENTS.forEach(function(evName) {
        api.on(evName + '.jquery', function(ev) {
          root.trigger.call(root, ev, ev.detail.args);
        });
      });
    });
  };
         ...
         speed: function(val, callback) {
            ...
            if (api.ready) {
               ...
               if (typeof val == "boolean") {
                  val = api.speeds[api.speeds.indexOf(api.currentSpeed) + (val ? 1 : -1)] || api.currentSpeed;
               }
      ...
      var selectEngine = function(clip) {
        ...
        if (conf.engine) {
          ...
          if (eng && clip.sources.some(function(source) {
            return eng.canPlay(source.type);
          })) return eng;
        }
         ...
         api.on('boot', function() {
            ...
            if (conf.splash) {
              Sizzle('.flowplayer.is-ready,.flowplayer.is-loading').forEach(function(el) {
                ...
                var a = instances[Number(playerId) - 1];
            ...
            var hlsFix = false;
            if (/mpegurl/i.exec(video.type)) {
              api.speeds = conf.speeds.filter(function(sp) { return sp <= 1; });
              // see https://github.com/flowplayer/flowplayer/issues/479
              hlsFix = api.conf.hlsFix;
            }
