  ...
  var root = element,
      ...
      var api = {
         ...
         load: function(video, callback) {
            ...
            if (video.src) {
               var e = new CustomEvent('load', {cancelable: true, bubbles: true, detail: {api: api, video: video, engine: engine}});
               api.trigger(e);
      ...
      ['on', 'one'].forEach(function(key) {
        api[key] = function(evType, slctr, hndlr) {
          ...
          bean[key](root, evType, function() {
            ...
            if (ev.detail && ev.detail.api) args = args.concat([ev.detail.api, ev.detail.video]).concat(additional);
